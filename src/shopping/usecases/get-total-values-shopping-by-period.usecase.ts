...
export class GetTotalValueShoppingByPeriodUseCase {
  constructor(private readonly salesService: ShoppingService) {}
  ...
  async execute(period: string) {
    try {
      ...
      let end: Date;
      if (period == 'all') {
        const response = await this.salesService.getTotalValueShopping();
        return {
          success: true,
          data: response,
        };
      ...
      const response = await this.salesService.getTotalValueShoppingByPeriod(
        beginning,
        end,
      );
      return {
